...
* This script assign the local anisotropies (in the form of 3-D ellipsoid rotation angles) to the samples and use them to calculate the experimental correlograms along the three main axes
...
In Windows, the script should run only with a double-click on the file. If it does not work, it is recommended to run the scripts using the terminal. Open the terminal in the folder with the repository files and run one of the desired Python scripts:
...
* The spatial autocorrelation is calculated using correlograms. But they are plotted upside down
